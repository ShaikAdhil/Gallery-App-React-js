...
class Gallery extends Component {
  state = {activeThunbnailId: imagesList[0].id}

  setActiveThumbnailId = id => {
    // console.log('clicked')
    this.setState({activeThunbnailId: id})
  }

  render() {
    const {activeThunbnailId} = this.state
    const {imageUrl, imageAltText} = imagesList[activeThunbnailId]
    return (
          ...
          <ul className="thumbnails-list">
            {imagesList.map(eachImage => (
              <ThumbnailItem
                ...
                isActive={activeThunbnailId === eachImage.id}
              />
            ))}
          </ul>
        </div>
